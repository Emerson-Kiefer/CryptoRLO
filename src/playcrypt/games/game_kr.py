...
class GameKR(Game):
    ...
    """
    def __init__(self, queries, block_cipher, key_len, block_len):
        """
        ...
        self.queries, self.block_cipher, self.key_len, self.block_len = queries, block_cipher, key_len, block_len
        self.key = ''
        self.count = 0
        self.messages = {}
        self.cyphers = {}
    ...
    def fn(self, m, k = None):
        if k == None:
            k = self.key
        self.messages[self.count] = m
        self.cyphers[self.count] = self.block_cipher.ciphers[(k, m)]
        self.count += 1
